...
# -------------------
hal_col = "HalID"
auteurs_fr_col = "Auteurs_FR"
auteurs_copub_col = "Auteurs_copubliants"
ville_col = "Ville_en_fr"
org_col = "Organisme_copubliant"
annee_col = "Année"
equipe_col = "Equipe"
...
def build_graph(df):
    G = nx.Graph()
    for _, row in df.dropna(subset=[auteurs_fr_col, auteurs_copub_col, ville_col]).iterrows():
...
# Onglets
# -------------------
tab1, tab2, tab3 = st.tabs(["Visualisation générale", "Réseau copublication", "Carte copublications Italie"])
...
with tab1:
    ...
    fig_year = px.bar(pubs_year, x=annee_col, y=hal_col, color=annee_col, title="Publications par année")
    ...
    if "Mots-cles" in df_filtered.columns:
        ...
        if text:
            with st.expander("Nuage de mots"):
...
with tab2:
    ...
    with st.expander("Afficher le réseau"):
...
# -------------------
# Onglet 3 : Carte interactive Italie (corrigé)
# -------------------
with tab3:
    st.header("Carte copublications Italie")
    with st.expander("Afficher la carte"):
        df_map = df_filtered.dropna(subset=["Latitude", "Longitude"])
        if df_map.empty:
            st.warning("Aucune donnée valide pour tracer la carte.")
        else:
            inria_lat, inria_lon = 43.619, 7.071
            df_map["Latitude_Inria"] = inria_lat
            df_map["Longitude_Inria"] = inria_lon
            ...
            for _, row in pubs_villes.iterrows():
                fig_map.add_trace(go.Scattermapbox(
                    ...
                    text=f"{row[ville_col]} : {row[hal_col]} publications",
                    hoverinfo="text", name=row[ville_col]
                ))

            arcs = df_map.groupby([auteurs_fr_col, auteurs_copub_col]).agg({
                "Latitude_Inria": "first", "Longitude_Inria": "first",
                "Latitude": "first", "Longitude": "first", hal_col: "count"
            }).reset_index()
            max_arc = arcs[hal_col].max()

            for _, row in arcs.iterrows():
                lon0, lat0 = row["Longitude_Inria"], row["Latitude_Inria"]
                ...
                fig_map.add_trace(go.Scattermapbox(
                    ...
                    line=dict(width=0.5 + (row[hal_col]/max_arc)*6,
                              color=f"rgba(30,144,255,{0.3 + 0.7*(row[hal_col]/max_arc)})"),
                    opacity=0.7, hoverinfo="text",
                    text=f"{row[auteurs_fr_col]} → {row[auteurs_copub_col]} : {row[hal_col]} pubs",
                    showlegend=False
                ))

            fig_map.add_trace(go.Scattermapbox(
                lon=df_map["Longitude_Inria"], lat=df_map["Latitude_Inria"],
                mode="markers", marker=dict(size=15, color="red", symbol="star"),
                text=df_map[auteurs_fr_col], hoverinfo="text", name="Inria"
            ))

            fig_map.update_layout(
                mapbox=dict(style="carto-positron", center=dict(lat=42.5, lon=12.5), zoom=5),
                margin=dict(l=0, r=0, t=50, b=0),
                title="Réseau géographique des copublications Inria - Italie"
            )
